...
def send_message(user_id, user_name, user_info, subject, body, attachments=[], headers={}):
  email = user_id
  name = user_name or user_info.get('name', email)
  recipient = formataddr((name, email))
  message = EmailMessage(subject, body, settings.SERVER_EMAIL, [recipient], headers=headers)
